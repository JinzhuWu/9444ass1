...
def my_perceptron(x):
    ...
    with tf.Session() as sess:
        sess.run(init)
        pre = tf.multiply(i,w)
        sum_result = tf.reduce_sum(pre)
        out = my_relu(sum_result)
...
def onelayer(X, Y, layersize=10):
    ...
    """
    # weights and biases
    # The initial weight is a random number from -1 to 1.
    w =  tf.Variable(tf.random_uniform([784,layersize],-1.0,1.0))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    """
    # first layer
    w1 =  tf.Variable(tf.random_uniform([784,hiddensize],-1.0,1.0))
    b1 =  tf.Variable(tf.zeros([hiddensize]))
    first_logits = tf.matmul(X,w1) + b1
    first_preds = tf.nn.relu(first_logits)
    # second layer
    w2 = tf.Variable(tf.random_uniform([hiddensize,outputsize],-1.0,1.0))
    b2 = tf.Variable(tf.zeros([outputsize]))
    logits = tf.matmul(first_preds,w2) + b2
    preds = tf.nn.softmax(logits)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
 ...
 #   print(height)
 #   print(width)
    # flat = tf.reshape(conv2, [-1, conv2.shape[1] * conv2.shape[2] * conv2.shape[3]])
    flat = tf.reshape(conv2, [-1, height * width * depth])

    w = tf.Variable(tf.truncated_normal([int(flat.shape[1]),outputsize]))
